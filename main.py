...
#assign the png to powerup
powerup = pygame.image.load("power.png")
...
# powerup xy and speed variable
powerX = random.randint(0, width - powerup.get_width())
powerY = random.randint(0, height - powerup.get_height())
speedMultiplier = 1
...
while gameover == False:
  for event in pygame.event.get():
    ...
    #declaring coords outside of loop
    coords = pygame.mouse.get_pos()
    
    # checking for powerup click
    if pygame.mouse.get_pressed()[0]:
        coords = pygame.mouse.get_pos()
        if coords[0] >= powerX and coords[0] <= powerX + powerup.get_width() \
        and coords[1] >= powerY and coords[1] <= powerY + powerup.get_height():
            #increase speed multiplier everytime the powerup is clicked
            speedMultiplier += .3
            powerX = random.randint(0, width - powerup.get_width())
            powerY = random.randint(0, height - powerup.get_height())
  ...
  if replayscreen == False:
    ...
    while iterator < numofchickens:
      if startY[iterator] + chicken.get_height() > height:
        replayscreen = True
        break
      startY[iterator] += speed[iterator] * speedMultiplier # added so the powerup speed multiplier affects it
      iterator += 1
